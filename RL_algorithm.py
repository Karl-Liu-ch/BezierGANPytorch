...
# env = OptimEnv()
# env = HicksHenneEnv()
env = AirfoilEnv(use_xfoil=opt.xfoil)

if opt.agent == 'ppo':
    # model = PPO("MlpPolicy", env, verbose=1)
    model = PPO("MlpPolicy", env, batch_size=256, policy_kwargs=policy_kwargs, verbose=1)
    path = '/work3/s212645/BezierGANPytorch/PPO/stablebaseline_ppo'
    try:
        model.load(path)
    ...
    path = '/work3/s212645/BezierGANPytorch/PPO/sac'
    model = SAC("MlpPolicy", env, learning_rate=3e-5, policy_kwargs=policy_kwargs, verbose=1)
    ...
    path = '/work3/s212645/BezierGANPytorch/PPO/td3'
